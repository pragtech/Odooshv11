...
#         print("CM-33-GREY,CMV-24-BEIGE and CMV-27-GREY",response_list)
...
#             print("SellerSKUSellerSKUSellerSKUSellerSKUSellerSKU",sku)
#             asin = response.get('asin')
#             logger.info("prod_data_list.get(sku)........................%s", prod_data_list.get(sku))
#             logger.info("response.......................%s", response)
#             list_price = prod_data_list.get(sku) and float(prod_data_list.get(sku).get('list_price').strip()) or 0.00
#             print("list_price",list_price)
#             fulfillment = prod_data_list.get(sku) and prod_data_list.get(sku).get('fulfillment-channel') or False
...
#             attributes =[]
#             variant_list = []
#             product_tmpl_ids = prod_temp_obj.search(['|',('amazon_sku','=',sku),('default_code','=',sku)])
...
#                         else:
#                             attribute_id = prod_att_obj.create({'name':attribute,'create_variant':True})
#                         attribute_values = []
...
#             else:
#                 prodtempl_vals.update({'fulfillment_by' : 'MFN'})
#             prodtempl_vals.update({'feature' : response.get('features', False)})
#             print("response.get('AttributeSets', [])",response.get('AttributeSets'))
...
#                    'amazon_active_prod':True,
#                    'amazon_product':True,
#                    'type':'product',
#                 })
# #             products = prod_obj.search([('default_code','=',sku)])     
# #             if products:
#             products = prod_obj.search([('default_code','=',sku)]) 
#             print("products@@@@@@@@@@@@@@@@@@@@@@",products)
#             if products:
#                 product_ids = prod_obj.search([('asin','=',asin)]) 
#                 print("product_idsproduct_idsproduct_ids",product_ids)
#                 if product_ids :
#                     if fulfillment=='AMAZON_NA':   
#                         prodtempl_vals.update({
#                             'amazon_fba_price' : list_price,
#                             'amazon_afn_sku':sku,
#                             'fulfillment_channel' :'FBA',
#                             'asin':asin
#                         })
#                         print("prodtempl_vals111111",prodtempl_vals)
#                         product_ids[0].write(prodtempl_vals)
#                         product_tmpl_id = product_ids[0].id
#                     else: 
#                         prodtempl_vals.update({
#                             'amazon_standard_price' : list_price,
#                             'asin':asin
#                         })
#                         print("prodtempl_vals2222222",prodtempl_vals)
#                         product_ids[0].write(prodtempl_vals)
#                         product_tmpl_id = product_ids[0].id
#                 if not product_ids:
#                     if fulfillment=='AMAZON_NA':
...
#                             'asin':asin,
#                         })
#                         print("prodtempl_vals3333333",prodtempl_vals)
#                         logger.info('prodtempl_vals amazonFBA-----------------------  %s', prodtempl_vals)
#                         products.write(prodtempl_vals)
# #                         logger.info("product_tmpl_id amazonFBA %s", product_tmpl_id)
#                     else:
#                         prodtempl_vals.update({
#                             'amazon_standard_price' : list_price,
#                             'asin':asin,
#                         })
#                         print("prodtempl_vals444444",prodtempl_vals)
...
#                             'type':'product',
#                         })
#                         print("prodtempl_vals1111elselllllll",prodtempl_vals)
#                         product_ids[0].write(prodtempl_vals)
#                         product_tmpl_id = product_ids[0].id
#                     else: 
#                         prodtempl_vals.update({
...
#                             
#                         })
#                         print("prodtempl_vals2222222lllllllllllll",prodtempl_vals)
#                         product_ids[0].write(prodtempl_vals)
#                         product_tmpl_id = product_ids[0].id
#                 else:
#                     if fulfillment=='AMAZON_NA': 
...
#                         })
#                         print("prodtempl_vals1111elselllllll",prodtempl_vals)
#                         product_tmpl_id = prod_obj.create(prodtempl_vals).id
#                     else: 
#                         title = response.get('Title', False)  
#                         if not name:
#                             name =  prodtempl_vals.get('label') 
                    ...
                    for product_data in product_ids:
                        ...
                        for product_info in response.get('variant_info'):
                            ...
                            for p_val in product_data.attribute_value_ids:
                                print (p_val.name)
                                print("product_info.get(p_val.attribute_id.name)",product_info.get(p_val.attribute_id.name))
                                if product_info.get(p_val.attribute_id.name) != p_val.name:
                                    flag = 0
                            if flag == 1:
                                asin_var = product_info.get('asin')
                                if not product_data.default_code ==sku[0:
                                    product_data.write({'default_code':sku[0],'asin':asin_var, 'amazon_product':True})   
...
#             try:
    ...
    def ImportAFNInventory(self, get_list=[], prod_data_list='',marketplace=''):
        ...
        for sku in get_list:
            print("sku",sku)
            prod_ids = product_obj.search([('amazon_afn_sku', '=', sku)])
            print("prod_ids",prod_ids)
            data = prod_data_list.get(sku).get('quantity')
            print("data================adttttttttttttttttttttttttasdtttttt ",data)
            print("prod_data_list.get(sku).get('quantity')prod_data_list.get(sku).get('quantity') ",prod_data_list.get(sku).get('quantity'))
...
#                 'tax_id': []
            ...
            tax_id = []
            if each_result.get('ItemTax',0) > 0.0:
            ...
            if resultval != {} and resultval.get('AmazonOrderId'):
                ...
                # Get Order Item Detail from Amazon using AmazonOrderID
                    ...
                    if resultval.get('PurchaseDate', False):
                        ...
                        do = datetime.strptime(resultval.get('PurchaseDate'), "%Y-%m-%dT%H:%M:%SZ")
                        print("dododo",do)
...
#             try:
            if self._context.get('last_amazon_order_import_date'):
                sdate = datetime.strptime(self._context.get('last_amazon_order_import_date'), "%Y-%m-%d %H:%M:%S")
...
#                     tz_name = self.env.context.get('tz') or self.env.user.tz
#                     today_utc = pytz.timezone('UTC').localize(sdate, is_dst=False)
...
#                     price_string += """<Message><MessageID>%s</MessageID><Price><SKU>%s</SKU><StandardPrice currency="%s">%.2f</StandardPrice></Price>
#                             </Message>"""% (message_id, amazon_list_data.default_code, marketplace.res_currency.name,amazon_list_data.price)
#                     message_id += 1
# 
...
#                                                 %s<DescriptionData>
#                                                 <Title>%s</Title>"""%(message_id,product_tem_data.amazon_sku,standard_product_string,date_string,title)
...
#                 bullets += """<BulletPoint>%s</BulletPoint>"""%(bullet_point.bullet)
#             message_information += bullets
#             
#         if product_tem_data.amazon_manufacturer:
#             message_information +="""<Manufacturer>%s</Manufacturer>"""%(product_tem_data.amazon_manufacturer)
# #                 message_information += desc
#         search_term = ''
#         if product_tem_data.search_keywords:
#             for searchterm_data in product_tem_data.search_keywords:
#                 search_term += """<SearchTerms>%s</SearchTerms>""" %(searchterm_data.searchterm)
...
# #         if standard_product_string:
# #             if product_tem_data.ean_barcode and product_tem_data.standard_temp_product=='EAN':
...
#                                                 <ProductTaxCode>A_GEN_NOTAX</ProductTaxCode>
#                                                 %s<DescriptionData>
#                                                 <Title>%s</Title>"""%(message_id,product_tem_data.amazon_sku,date_string,title)
#                     print("message_information",message_information)
#                     
#                     if product_tem_data.brand_name:
#                         message_information += """<Brand>%s</Brand>"""%(product_tem_data.brand_name)
...
#                     print("<<<<<<<<<<<<<<<<product_tem_data>>>>>>>>>>>>>>>",product_tem_data)
#                     message_information +="""  </DescriptionData>
#                                                     <ProductData>
#                                                     <HomeImprovement>
#                                                    <ProductType>
...
#                                                   </HomeImprovement>
#                                                   </ProductData>
#                                                   """%(product_tem_data.variationtheme)
#                     message_information += """</Product>
#                                                 </Message>"""
